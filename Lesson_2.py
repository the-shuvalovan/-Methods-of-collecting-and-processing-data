...
#Находим зарплату и разбиваем на значения "от", "до" и "валюта".
        ...
        if not compensation:
            ...
            if compensation[0] == 'до':
                compensation_min = None
                compensation_max = str(compensation[1]).replace('\u202f', ' ')
                compensation_currency = compensation[2]
            elif compensation[0] == 'от':
                compensation_min = str(compensation[1]).replace('\u202f', ' ')
                compensation_max = None
                compensation_currency = compensation[2]
            else:
                compensation_min = str(compensation[0]).replace('\u202f', ' ')
                compensation_max = str(compensation[2]).replace('\u202f', ' ')
